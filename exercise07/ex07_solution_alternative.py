...
fig, ax = plt.subplots(2,5,dpi=200, figsize=(9,4))
for i in range(0, len(e)):
    res = integrator.plot_wavefunction(grid, C[:,i], cgfs).reshape((len(z), len(x)))
    ix = int(i % 5)
    iy = int(i / 5)
    ax[iy,ix].imshow(res, origin='lower', extent=[-2,2,-2,2], cmap='PiYG')
    ax[iy,ix].set_xlabel('x [a.u.]')
    ax[iy,ix].set_ylabel('z [a.u.]')
    ax[iy,ix].set_title('MO #%i of CO' % (i+1))
    ax[iy,ix].grid(linestyle='--', alpha=0.5)
plt.tight_layout()